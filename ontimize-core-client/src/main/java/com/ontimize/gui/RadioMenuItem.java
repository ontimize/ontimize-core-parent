...
StatusComponent, HasPreferenceComponent, IDynamicItem, Freeable {
    ...
    /**
	 * The class constructor. Calls to super, initializes parameters and permissions and sets margin. <p>
	 * 
	 * @param parameters
	 *            the map with parameters
	 */
	public RadioMenuItem(Map<Object, Object> parameters) {
		super();
	 ...
	 * 
	 * @param parameters
	 *            the <code>Map</code> with parameters. The next parameters are added: <p> <Table BORDER=1 CELLPADDING=3 CELLSPACING=1 RULES=ROWS FRAME=BOX> <tr>
	 *            <td><b>attribute</td> <td><b>values</td> <td><b>default</td> <td><b>required</td> <td><b>meaning</td> </tr> <tr> <td>attr</td> <td></td> <td></td> <td>yes</td>
	 *            <td>The attribute values.</td> </tr> <tr> <td>status</td> <td></td> <td></td> <td>no</td> <td>Indicates the status of component.</td> </tr> <tr> <td>checked</td>
	 *            <td>yes/no</td> <td>no</td> <td>no</td> <td>Indicates the type of separator.</td> </tr> <tr> <td>mnemonic</td> <td></td> <td></td> <td>no</td> <td>Indicates the
	 *            mnemonic of field.</td> </tr> <tr> <td>shortcut</td> <td></td> <td></td> <td>no</td> <td>Indicates a shortcut to radio menu.</td> </tr> </Table>
	 */
	@Override
	...
	public void setResourceBundle(ResourceBundle resources) {
		try {
			...
			if (com.ontimize.gui.ApplicationManager.DEBUG) {
				RadioMenuItem.logger.debug(null, exc);
			} else {
				RadioMenuItem.logger.trace(null, exc);
			}
		}
	}
	...
	public void free() {
		...
		for (EventListener listener : listeners) {
			this.removeActionListener((ActionListener) listener);
		}
		MenuElement[] menuChildren = this.getSubElements();
		// Free the menu components
		for (MenuElement element : menuChildren) {
			if (element instanceof Freeable) {
				try {
					((Freeable) element).free();
				} catch (Exception e) {
					if (com.ontimize.gui.ApplicationManager.DEBUG) {
						RadioMenuItem.logger
						.debug("Exception in free() method " + element.getClass().toString(), e);
					} else {
						RadioMenuItem.logger
						.trace("Exception in free() method " + element.getClass().toString(), e);
					}
				}
			}
		}
