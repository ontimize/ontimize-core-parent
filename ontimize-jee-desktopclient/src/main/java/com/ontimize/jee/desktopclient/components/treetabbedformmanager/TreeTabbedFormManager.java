...
public class TreeTabbedFormManager extends BaseFormManager implements ITreeTabbedFormManager {
	 ...
	 * </Table>
	 */
	public TreeTabbedFormManager(Map<Object, Object> parameters) throws Exception {
		super(parameters);
	}
			...
			if (!this.isLoaded()) {
				...
				try {
					...
					if (this.treeFileName != null) {
						try {
							Form mockForm = new Form(new HashMap<>());
	...
	public List<String> getTextsToTranslate() {
		...
		if (this.managedTreeTableColumn != null) {
			v.addAll((List<String>)this.managedTreeTableColumn.getTextsToTranslate());
		}
		return v;
	}
		...
		if (this.resourceFileName == null) {
			...
			if (resources != null) {
				...
				try {
					...
					TreeTabbedFormManager.logger.debug("{}", exc.getMessage(), exc);
				}
			}
	...
	public void setApplicationPreferences(ApplicationPreferences ap) {
		...
		this.registerApplicationPreferencesListener();
		Iterator<String> enumKeys = this.formReferenceList.keySet().iterator();
		while (enumKeys.hasNext()) {
			String oKey = enumKeys.next();
	...
	public int indexOf(TreeTabbedDetailForm searchedForm) {
		Map<Object, Object> keyValues = searchedForm.getKeysValues();
			...
			if (component instanceof TreeTabbedDetailForm) {
				...
				if ((detailForm.getEntityName() != null) && detailForm.getEntityName().equals(entityName)) {
					if ((detailForm.getMode() == InteractionManager.UPDATE)
						...
						List<?> keyAttrs = currentForm.getKeys();
						boolean check = true;
