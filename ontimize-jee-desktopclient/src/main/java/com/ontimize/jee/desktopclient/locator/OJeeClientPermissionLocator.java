 ...
 */
public class OJeeClientPermissionLocator
		implements SecureEntityReferenceLocator, ConnectionOptimizer, XMLClientProvider, UtilReferenceLocator, ClientPermissionManager, RemoteApplicationPreferenceReferencer,
	...
	public OJeeClientPermissionLocator(Map<Object, Object> params) {
	...
	public void initializeParameters(Map<Object, Object> params) {
	...
	private void configureClientPermissionColumn(Map<Object, Object> params) {
		Object col = ApplicationManager.getParameterValue(OJeeClientPermissionLocator.CLIENT_PERMISSION_COLUMN, params);
		if (col != null) {
			this.clientPermissionsColumn = col.toString();
		}
	}

	private void configureRemoteReferenceLocatorName(Map<Object, Object> params) {
		Object oRemoteReferenceLocatorName = ApplicationManager.getParameterValue(OJeeClientPermissionLocator.REMOTE_REFERENCE_LOCATOR_NAME, params);
		if (oRemoteReferenceLocatorName != null) {
			...
			OJeeClientPermissionLocator.logger.error("'{}' parameter not found", OJeeClientPermissionLocator.REMOTE_REFERENCE_LOCATOR_NAME);
		}
		...
		if ((sRemoteProperty != null) && (!this.localLocator)) {
			...
			OJeeClientPermissionLocator.logger.info("Using '{}'.", OJeeClientPermissionLocator.REMOTE_LOCATOR_NAME_PROPERTY);
		}
	}

	private void checkMessageServerPeriod(Map<Object, Object> params) {
		Object checkservermessagesperiod = ApplicationManager.getParameterValue(OJeeClientPermissionLocator.CHECK_SERVER_MESSAGES_PERIOD, params);
		if (checkservermessagesperiod != null) {
			try {
				...
				OJeeClientPermissionLocator.logger.error("'" + OJeeClientPermissionLocator.CHECK_SERVER_MESSAGES_PERIOD + "' parameter error.", e);
			}
		}
	}
	...
	public void showMessageDialog(Component component) {
		...
		if (this.chatWindow == null) {
			...
			tUser.setResourceBundle(ApplicationManager.getApplication() != null ? ApplicationManager.getApplication().getResourceBundle() : null);
			tUser.setAdvancedQueryMode(true);

			tUser.setReferenceLocator(this);

			southPanel.add(tUser, new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			southPanel.add(new JScrollPane(this.text), new GridBagConstraints(0, 1, 2, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

			southPanel.add(this.b, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0));
			this.chatWindow.getContentPane().add(southPanel,
					new GridBagConstraints(0, 1, 1, 1, 0, 0.1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
			...
			this.b.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						if ((OJeeClientPermissionLocator.this.text.getText() != null) && (OJeeClientPermissionLocator.this.text.getText().length() > 0) && !tUser.isEmpty()) {
							...
							try {
								...
								OJeeClientPermissionLocator.this.s.getVerticalScrollBar().setValue(OJeeClientPermissionLocator.this.s.getVerticalScrollBar().getMaximum());
							} catch (Exception ex) {
							...
							if (usu == null) {
								...
								if (oValue instanceof List) {
									for (int i = 0; i < ((List<?>) oValue).size(); i++) {
										Object v = ((List<?>) oValue).get(i);
										OJeeClientPermissionLocator.this.sendMessage(m, (String) v, OJeeClientPermissionLocator.this.getSessionId());
									}
								} else if (oValue instanceof String) {
									OJeeClientPermissionLocator.this.sendMessage(m, (String) oValue, OJeeClientPermissionLocator.this.getSessionId());
								}
							} else {
								OJeeClientPermissionLocator.this.sendMessage(m, (String) usu, OJeeClientPermissionLocator.this.getSessionId());
							}

						}
					} catch (Exception ex) {
						OJeeClientPermissionLocator.logger.error(null, ex);
						MessageDialog.showMessage(OJeeClientPermissionLocator.this.chatWindow, ex.getMessage(), JOptionPane.ERROR_MESSAGE,
								ApplicationManager.getApplication() != null ? ApplicationManager.getApplication().getResourceBundle() : null);
					}
				}
			});
